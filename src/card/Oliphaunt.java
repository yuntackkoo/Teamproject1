...
public class Oliphaunt extends Pawn
{
	//20
	public Oliphaunt()
	{
	...
	public void effect(ServerPlayer hostp, ServerPlayer theyp, int target, boolean spcon)
	{
		if(spcon) // 타겟이 필요하면?
		{
			this.setCurrentatt(this.getCurrentatt()+1);
	...
	public boolean spcondition(ClientPlayer p)
	{
		...
		{
			if(p.getFieldlist().getFiled().get(i).getCardNumber() == 9) // 자기 핸드, 필드 , 상대 필드 까지 참고가능
			{
				super.setSpcon(true);
				return true;
			}
